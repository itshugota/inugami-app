...
class ToDoService {
  ...
  async fetchToDos() {
    ...
    if (this.toDos.length === 0 || !this.$cacheFactory.get('$http').get(urlJoin(BACKEND_SERVER_BASE_URL, '/api/to-dos'))) {
  ...
  async updateToDoById(id, updateInfo, shouldNotifyObservers = true) {
    const response = await this.$http.put(urlJoin(BACKEND_SERVER_BASE_URL, `/api/to-dos/${id}`), updateInfo);
  ...
  async updateToDoFocusTime(id, isCountingFocusTime, shouldNotifyObservers = true) {
    const response = await this.$http.put(urlJoin(BACKEND_SERVER_BASE_URL, `/api/to-dos/${id}/focus-time`), { isCountingFocusTime });
...
ToDoService.$inject = ['$http', '$cacheFactory'];
