...
class AuthController {
  ...
  private loginUser = async (req: Request, res: Response) => {
    ...
    const doesPasswordMatch = user.password ? await bcrypt.compare(password, user.password) : false;
    ...
    if (doesPasswordMatch) {
      ...
      const authToken = jwt.sign(jwtPayload, keys.secretOrKey, { expiresIn: 86400 });
